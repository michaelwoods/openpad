...
describe("Editor", () => {
  ...
  it("calls handleGenerate with the correct parameters on button click", async () => {
    const handleGenerateSpy = vi
      .spyOn(api, "handleGenerate")
    ...
    await act(async () => {
      fireEvent.keyDown(
        screen.getByPlaceholderText(
          "e.g., a 20mm cube with a 5mm hole in the center",
        ),
  ...
  it("updates the attachment state when a file is uploaded", async () => {
    render(<Editor />);
    const file = new File(["cylinder(10);"], "test.scad", {
      type: "text/plain",
    });
  ...
  it("adds to history on successful generation", async () => {
    vi.spyOn(api, "handleGenerate").mockImplementation(
      async (_p, _m, _prov, _sil, _ssd, _sgc, _sgi, _ec, _s, _a, onSuccess) => {
